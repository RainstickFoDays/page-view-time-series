...
def draw_bar_plot():
    ...
    df_bar.index = df_bar.index.to_period('M')
    ...
    sns.barplot(data=df_bar, x='year', y='value', hue='Months', ax = ax, palette = 'tab10')
    ax.set_xlabel('Years')
...
def draw_box_plot():
    ...
    sns.boxplot(data=df_box, x='year', y='value', hue='year', ax=ax1, \
                fliersize=1, palette='tab10', legend=False, gap=0.2)
    ...
    sns.boxplot(data=df_box, x='month', y='value', hue='month', ax=ax2, \
                fliersize=1, palette='pastel', legend=False, gap=0.2,\
